    ...
    public async Task<int> SyncEventsFromExternalApi(string? keyword = null, string? segmentName = null)
    {
        ...
        foreach (var tmEvent in tmData._embedded.events)
        {
            ...
            var newEvent = new Event
            {
                ...
                Venue = tmEvent._embedded?.venues?.FirstOrDefault()?.name ?? "Unknown Venue",
                City = tmEvent._embedded?.venues?.FirstOrDefault()?.city?.name ?? "Unknown City",
                Genre = tmEvent.classifications?.FirstOrDefault()?.genre?.name,
                ImageUrl = tmEvent.images?.FirstOrDefault(i => i.width > 1000)?.url ?? tmEvent.images?.FirstOrDefault(i => i.width > 500)?.url ?? tmEvent.images?.FirstOrDefault()?.url ?? "",
